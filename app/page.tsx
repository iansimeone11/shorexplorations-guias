...
const logo = `${assetBase}/logo-transparent.png`;
...
function pdfUrl(file: string) { return `${assetBase}/pdfs/${encodeURIComponent(file)}`; }
...
export default function Home() {
  ...
  if (!assignment) {
    return <main className="login-page"><section className="login-brand-panel"><img src={logo} alt="Shorexplorations" /><div><p className="eyebrow">Portal operativo</p><h1>Tu servicio.<br />Toda la informacion.</h1><p>Consulta tu tour asignado, bus, punto de partida y descriptivo antes de comenzar.</p></div><span className="login-edition">Buenos Aires - Guias</span></section><section className="login-form-panel"><form onSubmit={login}><p className="eyebrow">Acceso de guias</p><h2>Iniciar sesion</h2><p className="form-intro">Ingresa tu apellido y los ultimos cuatro numeros de tu DNI.</p><label><span>Apellido</span><input autoComplete="username" value={lastName} onChange={(event) => setLastName(event.target.value)} placeholder="Ej. Gonzalez" required /></label><label><span>Ultimos 4 del DNI</span><input autoComplete="current-password" inputMode="numeric" type="password" maxLength={4} pattern="[0-9]{4}" value={pin} onChange={(event) => setPin(event.target.value.replace(/\D/g, ""))} placeholder="1234" required /></label>{loginError && <p className="login-error" role="alert">{loginError}</p>}<button className="login-button" type="submit">Ver mi asignacion <span aria-hidden="true">→</span></button><aside className="demo-access"><strong>Acceso de prueba</strong><span>Apellido: DEMO - Clave: 1234</span></aside></form></section></main>;
  }

  return <main className="portal-page"><header className="portal-topbar"><a className="portal-brand" href="#asignacion"><img src={logo} alt="Shorexplorations" /></a><div className="guide-account"><span><strong>{assignment.guideFirstName} {assignment.guideLastName}</strong><small>Guia asignada</small></span><button type="button" onClick={() => setAssignment(null)}>Salir</button></div></header><section className="assignment-hero" id="asignacion"><div className="assignment-heading"><div><p className="eyebrow">{assignment.dateLabel}</p><h1>Buen dia,<br />{assignment.guideFirstName}.</h1></div><span className="status-pill"><i /> Servicio asignado</span></div><article className="primary-assignment"><div className="assignment-main"><div className="assignment-code"><span>Codigo</span><strong>{assignment.tour.code}</strong></div><p className="eyebrow">Tu tour de hoy</p><h2>{assignment.tour.title}</h2><p>{assignment.tour.summary}</p><div className="assignment-actions"><button type="button" onClick={() => setSelected(assignment.tour)}>Ver descriptivo <span>↗</span></button><a href={pdfUrl(assignment.tour.file)} target="_blank">Abrir PDF <span>↓</span></a></div></div><dl className="operation-grid"><div><dt>Bus asignado</dt><dd><b>{assignment.bus}</b></dd></div><div><dt>Presentacion</dt><dd>{assignment.meetingTime}</dd></div><div><dt>Salida</dt><dd>{assignment.startTime}</dd></div><div><dt>Punto de partida</dt><dd>{assignment.departurePoint}</dd></div></dl><div className="operation-alert"><span>Importante</span><p>{assignment.operationNote}</p></div></article></section><section className="quick-info"><article><span>01</span><div><strong>Llega con anticipacion</strong><p>La presentacion obligatoria en la puerta del bus es a las {assignment.meetingTime}, 30 minutos antes del inicio del tour ({assignment.startTime}).</p></div></article><article><span>02</span><div><strong>Verifica material y audio</strong><p>Controla descriptivo, vouchers, reservas, numero identificatorio y correcto funcionamiento del microfono antes del embarque.</p></div></article><article><span>03</span><div><strong>Avisa participantes reales</strong><p>Al comenzar, envia por WhatsApp a Ian la cantidad de participantes que efectivamente inicio el tour.</p></div></article><article><span>04</span><div><strong>Confirma el cierre</strong><p>Este servicio finaliza en {assignment.tour.endpoint}.</p></div></article></section><section className="portal-library"><div className="section-heading"><div><p className="eyebrow">Biblioteca operativa</p><h2>Todos los descriptivos</h2></div><p className="result-count">{filtered.length} resultados</p></div><div className="tools"><label className="search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por nombre o codigo..." />{query && <button type="button" onClick={() => setQuery("")}>×</button>}</label><div className="filters">{categories.map((item) => <button key={item} className={item === category ? "active" : ""} onClick={() => setCategory(item)} type="button">{item}</button>)}</div></div><div className="tour-grid">{filtered.map((tour, index) => <article className="tour-card" key={`${tour.file}-${index}`}><div className="card-topline"><span className="code">{tour.code}</span><span className="category">{tour.category}</span></div><h3>{tour.title}</h3><p>{tour.summary}</p><dl><div><dt>Duracion</dt><dd>{tour.duration}</dd></div><div><dt>Cierre</dt><dd>{tour.endpoint}</dd></div></dl><div className="card-actions"><button type="button" onClick={() => setSelected(tour)}>Ver online ↗</button><a href={pdfUrl(tour.file)} target="_blank">Abrir ↓</a></div></article>)}</div></section><footer><div><img src={logo} alt="" /><span>Shorexplorations</span></div><p>Confirma cambios operativos del dia antes de iniciar.</p></footer>{selected && <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setSelected(null)}><section className="modal"><div className="modal-header"><div><span>{selected.code} - {selected.duration}</span><h2>{selected.title}</h2></div><div className="modal-actions"><a href={pdfUrl(selected.file)} target="_blank">Abrir PDF ↓</a><button type="button" onClick={() => setSelected(null)}>×</button></div></div><iframe title={`Descriptivo: ${selected.title}`} src={`${pdfUrl(selected.file)}#view=FitH&toolbar=1`} /></section></div>}</main>;
}
